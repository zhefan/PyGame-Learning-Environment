...
class DotShooter(PyGameWrapper):
    ...
    def __init__(self, version=0, width=32, height=30, target_speed_ratio=1,
                 players_speed_ratio=1, bullet_speed_ratio=1, MAX_STEPS=150):
    ...
    def init(self):
        ...
        if self.version >= 6:  # random player
    ...
    def step(self, dt):
        ...
        for idx, bullet in enumerate(self.bullet_list):
            if self.version == 6 or self.version == 7:  # three targets
            ...
            if bullet_status >= 1:  # target hit
                ...
                if bullet_status == 1:  # hit main target
                    ...
                    if self.version == 6 or self.version == 7:
                        ...
                        raise ValueError('Bullet status and version error')
                self.score_counts['agent'] = self.score_sum
                self.target_hit = True
            elif bullet_status == 0:  # out of bound
                to_del.append(idx)
                self.bullet_group.remove(bullet)
                if self.version == 7 or self.version == 9:
